...
const captureScreenshot = async (element: HTMLElement): Promise<HTMLCanvasElement> => {
  ...
  try {
    ...
    for (let i = 0; i < styles.length; i++) {
      const prop = styles[i];
      clone.style[prop as any] = styles.getPropertyValue(prop);
    }
          ...
          if (!img.src.startsWith('data:')) {
            ...
            if (img.complete && img.naturalWidth > 0) {
              ...
              if (ctx) {
                ...
                console.log(`Converted cloned image from ${originalSrc} to data URL`);
              }
            }
          }
        } catch (err) {
...
export const useExportToPDF = () => {
  ...
  const exportElements = useCallback(async ({
    ...
    try {
      ...
      // Create the PDF document (used for both formats)
      const pdf = new jsPDF({
        orientation,
        unit: 'mm',
        format: 'a4',
